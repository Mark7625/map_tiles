...
def main():
    ...
    # Overall plane progress bar
    with tqdm.tqdm(total=MAX_Z + 1, desc="Planes", position=0) as overall_pbar:
...
def map_openrs2_xteas_to_runelite_format(xteas):
    return [{"region": region["mapsquare"], "keys": region["key"]} for region in xteas]
...
def build_full_map_images(cache_dir, xtea_file):
    ...
    subprocess.run(
        ['java', '-Xmx8g', '-cp', jar_file, 'org.explv.mapimage.Main',
         '--cachedir', cache_dir,
         '--xteapath', xtea_file,
...
def generate_tiles_for_plane(plane):
    ...
    image_width_tiles = new_image.width // TILE_SIZE_PX
    ...
    # Per-plane progress bar
    with tqdm.tqdm(total=len(changed_tiles), desc=f"Tiles Plane {plane}", position=1) as tile_pbar:
        ...
        for zoom in range(starting_zoom + 1, MAX_ZOOM + 1):
            ...
            tile_pbar.total = len(next_changed_tiles)
            tile_pbar.n = 0
            for tile in next_changed_tiles:
                tile_pbar.update(1)
        ...
        for zoom in reversed(range(MIN_ZOOM + 1, starting_zoom + 1)):
            ...
            tile_pbar.total = len(changed_tiles)
            tile_pbar.n = 0
            for tile in changed_tiles:
                tile_pbar.update(1)
    ...
    with thread_pool_executor() as executor:
        ...
        for tile_x in range(0, new_image_width_px, TILE_SIZE_PX):
            for tile_y in range(0, new_image_height_px, TILE_SIZE_PX):
                futures.append(
                    executor.submit(has_tile_changed, plane=plane, zoom=zoom, tile_x=tile_x, tile_y=tile_y,
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if has_changed:
                ...
                y = max_y - y - 1
                changed_tiles.append({"pixel_x": tile_x, "pixel_y": tile_y, "x": x, "y": y, "image": new_image_tile})
...
def has_tile_changed(plane, zoom, tile_x, tile_y, old_image, new_image):
    ...
    old_image_np = np.ndarray(buffer=old_image_tile.write_to_memory(),
                              dtype=np.uint8, shape=[old_image_tile.height, old_image_tile.width, old_image_tile.bands])
    new_image_np = np.ndarray(buffer=new_image_tile.write_to_memory(),
                              dtype=np.uint8, shape=[new_image_tile.height, new_image_tile.width, new_image_tile.bands])
...
def split_tiles_to_new_zoom(changed_tiles, plane, new_zoom):
    new_changed_tiles = []
    with thread_pool_executor() as executor:
        futures = [executor.submit(split_tile_to_new_zoom, changed_tile, plane, new_zoom) for changed_tile in changed_tiles]
        for future in concurrent.futures.as_completed(futures):
...
def split_tile_to_new_zoom(changed_tile, plane, new_zoom):
    original_x, original_y = changed_tile["x"], changed_tile["y"]
    tile_image = changed_tile["image"].resize(2, kernel='nearest')

    new_x, new_y = original_x * 2, original_y * 2

    tile_image_0 = tile_image.crop(0, TILE_SIZE_PX, TILE_SIZE_PX, TILE_SIZE_PX)
    save_tile(tile_image_0, plane, new_zoom, new_x, new_y)

    tile_image_1 = tile_image.crop(TILE_SIZE_PX, TILE_SIZE_PX, TILE_SIZE_PX, TILE_SIZE_PX)
    save_tile(tile_image_1, plane, new_zoom, new_x + 1, new_y)

    tile_image_2 = tile_image.crop(0, 0, TILE_SIZE_PX, TILE_SIZE_PX)
    save_tile(tile_image_2, plane, new_zoom, new_x, new_y + 1)

    tile_image_3 = tile_image.crop(TILE_SIZE_PX, 0, TILE_SIZE_PX, TILE_SIZE_PX)
    save_tile(tile_image_3, plane, new_zoom, new_x + 1, new_y + 1)

    return [
        {"x": new_x, "y": new_y, "image": tile_image_0},
        {"x": new_x + 1, "y": new_y, "image": tile_image_1},
        {"x": new_x, "y": new_y + 1, "image": tile_image_2},
        {"x": new_x + 1, "y": new_y + 1, "image": tile_image_3}
    ]
...
def join_tiles_to_new_zoom(changed_tiles, plane, current_zoom, new_zoom):
    new_changed_tiles = []
    with thread_pool_executor() as executor:
        futures = [executor.submit(join_changed_tile_to_new_zoom, changed_tile, plane, current_zoom, new_zoom)
...
def join_changed_tile_to_new_zoom(changed_tile, plane, current_zoom, new_zoom):
    ...
    return {"x": new_x, "y": new_y, "image": new_tile_image_resized}
...
def output_tile_diff_image(changed_tiles, new_image_location, output_file_name):
    im = Image.open(new_image_location)
